...
export const ActiveChatsContext = createContext<ContextType>({
  activeChats: null,
  activeChatsDispatch: () => null,
});

export function reducer(
  state: ActiveChatsType,
  action: ActionType,
): ActiveChatsType {
    ...
    case "DELETE_ACTIVE_CHAT":
      ...
      return {
        activeChats: [...state.activeChats!].filter(
          (item: UserField) => item._id !== disconntectedUser._id,
        ),
      };
...
export const ActiveChatsContextProvider = ({
  children,
}: {
  children: React.ReactNode;
}): JSX.Element => {
  const [activeChats, activeChatsDispatch] = useReducer(reducer, {
    activeChats: null,
  });

  return (
    <ActiveChatsContext.Provider
      value={{ ...activeChats, activeChatsDispatch }}
    >
      {children}
    </ActiveChatsContext.Provider>
  );
