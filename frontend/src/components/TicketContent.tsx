    ...
    useEffect(() => {
        if(currentUser && ticketNumber){
            const fetchTicketMessage = async() => {
                const response = await axios(`http://localhost:1913/api/message/getmessagebyticketnumber/${ticketNumber}`);
                ticketContentDispatch({type:"SET_TICKET_CONTENT", payload: response.data});
            };
            fetchTicketMessage();
        }
        ...
        <div id="ticket-content-main">
            ...
            <div id="ticket-content">
                {ticketContent && ticketContent.map(ticket => {
                    return(
                        <div key = {ticket._id}>
