...
export async function getTodosByUserId(req: Request, res: Response ) {
    ...
    try{
        ...
        if(todoList){
            ...
            //@ts-ignore
            todoList.tasks = todoList.tasks.sort((a,b) => (a.completed > b.completed) ? 1 : ((b.completed > a.completed) ? -1 : 0))
        }
...
export async function updateTodo(req: Request, res: Response) {
    ...
    try{
        const updatedTodoList = await TodoList.findOneAndUpdate({userId}, {
            tasks: tasks
        })
